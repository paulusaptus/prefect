...
describe("task run concurrency limits hooks", () => {
	 ...
	 */
	it("is stores list data into the appropriate list query when using useSuspenseQuery()", async () => {
	 ...
	 */
	it("is stores details data into the appropriate list query when using useSuspenseQuery()", async () => {
		// ------------ Mock API requests when cache is empty
		...
		const { result } = renderHook(
			() =>
				useSuspenseQuery(buildDetailTaskRunConcurrencyLimitsQuery(mockData.id)),
			{ wrapper: createQueryWrapper({}) },
		);
